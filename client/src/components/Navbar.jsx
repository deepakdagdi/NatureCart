...
const Navbar =()=>{
    ...
    const {user,setUser,setShowUserLogin,navigate,searchQuery,
          setSearchQuery,getCartCount,getCartAmount,axios}=useAppContext();
...
<NavLink to='/' onClick={()=> setOpen(false)}>
    <img className="h-14 w-40"src={assets.logo} alt="Logo" />
</NavLink>
...
<div className="hidden sm:flex items-center gap-8">
   ...
   <NavLink to="/seller">Admin</NavLink> 
    ...
    className="relative cursor-pointer">
        ...
        <button 
        className="absolute -top-2 -right-3 text-xs text-white bg-primary w-[18px] h-[18px] 
  ...
  className="cursor-pointer px-8 py-2 bg-primary hover:bg-primary-dull transition
   text-white rounded-full">
    ...
    <div className="relative group">
        <img src={assets.profile_icon} className="w-10" alt="profille"/>
